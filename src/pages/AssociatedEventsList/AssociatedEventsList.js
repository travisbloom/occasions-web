...
import AssociatedEventSummary, {
  AssociatedEventSummaryShell,
} from './AssociatedEventSummary';
...
class AssociatedEventsList extends React.Component {
  ...
  renderBody = () => (
    <View marginChildren>
      {this.props.data.currentUser.person.createdEvents.edges.map(
        ({node}, index) => (
          <AssociatedEventSummary
            key={node.id}
            index={index}
            associatedEvent={node}
          />
        )
      )}
    </View>
...
export default compose(
  ...
  withApolloFetchingContainer(() => (
    <View marginChildren>
      {new Array(4)
        .fill()
        .map((_, index) => <AssociatedEventSummaryShell key={index} />)}
    </View>
  ))
